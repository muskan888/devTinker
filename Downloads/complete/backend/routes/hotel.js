...
router.get("/all", (req, res) => {
  ...
  Hotel.find()
    // .populate("no_of_rooms", ["no_of_hotel", "available_of_rooms"])
    .then(hotels => {
      if (!hotels) {
...
router.post("/add", (req, res) => {
  ...
  // Get fields
  const hotelFields = {};
  if (req.body.name) hotelFields.name = req.body.name;
  ...
  Hotel.findOne({ no_of_rooms: req.body.no_of_rooms }).then(hotel => {
    if (hotel) {
      //Update the hotel
      Hotel.findOneAndUpdate({ no_of_rooms: req.body.no_of_rooms }, { $set: hotelFields }, { new: true }).then(
      ...
      Hotel.findOne({ name: hotelFields.name }).then(hotel => {
        if (hotel) {
